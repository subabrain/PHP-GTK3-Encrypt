...
std::vector<unsigned char> ReadEncryptedValueFromDLL(const char* dllPath) {
    std::ifstream dllFile(dllPath, std::ios::binary);
    if (!dllFile.is_open()) {
        throw std::runtime_error("Fehler beim Öffnen der DLL-Datei");
    }

    dllFile.seekg(0, std::ios::end);
    int fileSize = dllFile.tellg();
    dllFile.seekg(0, std::ios::beg);

    std::vector<unsigned char> encryptedValue(fileSize);
    dllFile.read(reinterpret_cast<char*>(&encryptedValue[0]), fileSize);
    dllFile.close();
...
std::string GenerateAesKey(const std::string& macAddress, const std::string& date) {
    // Kombiniere die MAC-Adresse und das Datum
    std::string combinedData = macAddress + date;

    // Berechne den Hash des kombinierten Datenstrings
...
std::string ConvertToString(const unsigned char* data, size_t size) {
...
void preprocessPHPCode(std::string& phpCode) {
    ...
    // Lesen des verschlüsselten Werts aus der DLL-Datei
    const char* dllPath = "../phpgtk/key.dll";

    std::vector<unsigned char> encryptedValue = ReadEncryptedValueFromDLL(dllPath);

    std::string byteString(encryptedValue.begin(), encryptedValue.end());

    // Entschlüsseln des Werts (ECB-Modus)
    unsigned char key[] = "V3oURkYgTDHpCf4c"; // Verwenden Sie den gleichen Schlüssel, den Sie in C# verwendet haben
    AES_KEY aesKey;

    AES_set_decrypt_key(key, 128, &aesKey);

    unsigned char decryptedValue[16]; // Hier gehen wir davon aus, dass der verschlüsselte Wert 16 Bytes lang ist
    AES_ecb_encrypt(&encryptedValue[0], decryptedValue, &aesKey, AES_DECRYPT);

    //std::string str_dec = ConvertToString(decryptedValue, sizeof(decryptedValue));
    ...
    std::string decryptedText = decryptAES_ECB(base64EncodedText, decryptedValue);
